	...
	public:
		...
		ScoredSeq(string seq, double as, unsigned long pos, int strand, int am, bool pe1, bool pe2) :
			...
			if(strand == NEG_STRAND)  {
				// already been done from Driver2.cpp ...
...
//			cout << "inserting: " << pos << endl;
		...
		void get_SAM2(double denom, Read &read, string consensus, Genome &gen, unsigned int rIndex, int &multiMatchIdx, map<unsigned long, unsigned int> &refMapLoc, vector<TopReadOutput> &ret_vec) {
			bin_seq bs;

			if(positions.size() == 0)
				return; //todo: print unmapped fastq too
			
			bool print_all_sam=false;
			if (multiMatchIdx<0) {
				print_all_sam=true;
			}
			...
			for(sit = positions.begin(); sit != positions.end(); sit++) {
				...
				if (refMapLoc[posInFile]>0) {
					...
					if ((!print_all_sam) && (multiMatchIdx > gMAX_MATCHES))
						break;
