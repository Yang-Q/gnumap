...
void create_match_output(GENOME_t &gen, unsigned int rIndex, string &consensus) {
	...
	bool skipMap = false;
	unsigned int multipleTopsCnt;
	unsigned int multipleTopsCntPos = 0;
	unsigned int multipleTopsCntNeg = 0;
	unsigned int total_max_match_allowed = gMAX_MATCHES;
...
int main(const int argc, const char* argv[]) {
	...
	*/
	...
	if(gMIN_JUMP_MATCHES < 1)
		usage(1, (char*)"-k/--num_seed: Invalid matching seed number\n");
	...
	if(gATOG) {
		...
		if (gMATCH_POS_STRAND_RE) {
			...
			if(pos_matrix == NULL) {
				...
				gALIGN_SCORES[(int)'a'][1] = gTRANSVERSION_bs;
				gALIGN_SCORES[(int)'a'][2] = 0; //we have to turn it off
				gALIGN_SCORES[(int)'a'][3] = gTRANSVERSION_bs;
			...
			// If they've supplied a matrix, we don't want to mess with this
			if(pos_matrix == NULL) {
				gALIGN_SCORES[(int)'t'][0] = gTRANSVERSION_bs;
				gALIGN_SCORES[(int)'t'][1] = 0;
				gALIGN_SCORES[(int)'t'][2] = gTRANSVERSION_bs;
...
int ParseCmdLine(const int argc, const char* argv[]) {
	...
	if (!gMATCH_POS_STRAND || !gMATCH_NEG_STRAND) {
		//gMAX_PRE_MATCHES = PRE_MATCH_BACKUP_FOLDS * gMAX_MATCHES;
		gMAX_PRE_MATCHES_HALF = 2 * PRE_MATCH_BACKUP_FOLDS_STRAND * gMAX_HASH_SIZE * gMAX_MATCHES;
	}
	else {
		gMAX_PRE_MATCHES_HALF = PRE_MATCH_BACKUP_FOLDS_STRAND * gMAX_HASH_SIZE * gMAX_MATCHES;
		//gMAX_PRE_MATCHES = PRE_MATCH_BACKUP_FOLDS * gMAX_MATCHES;
	}
	return 0;
...
unsigned int align_sequence(Genome &gen, seq_map &unique, const Read &search, const string &consensus, double min_align_score, double max_align_score2, double &denominator, double &top_align_score, int &cnt_summit, int strand2ali, int align_mode, int thread_id) {
	...
	unsigned long beginningE, beginning;
	...
	int jumpSize = (int)floor((searchL-gMER_SIZE)*(gMER_SIZE*0.5-1.0)/(gSEQ_LENGTH-gMER_SIZE)+1.5);
	
#ifdef DEBUG
	fprintf(stderr,"jumpSize=%d\n",jumpSize);
#endif
...
//The following While loop should be in a separate function
/*
	if (nwAlignF) {
		nw_align_prematch()
		if (cnt_my_map < 1)
			if (rnaF)
				spj_nw_align_prematch()
			else if (swF)
				sw_align_prematch()
	}
...
//Once we directly go to the loc_it(can be multiple) corresponding to maxHit by using op(find ) and traverse each loc by using (bidirectional) iterator
	
	for(loc_it = possible_locs.begin(); loc_it != possible_locs.end(); ++loc_it) {
		
		if (cnt_my_map>gMAX_PRE_MATCHES_HALF)
			break;
	...
	if (gSOFT_CLIP && (cnt_my_map == 0) && softClipFirstVisitF) {
		align_mode = SW_ALIGN;
		softClipFirstVisitF = false;
		swMinLen = (unsigned int)ceil(log(EVAL_K*gGenSizeBp0*searchL/gSW_Eval_cutoff)/(gMATCH*gEvalLambda/gADJUST));
		...
		if (swMinLen > searchL) {
			...
			/*
			if (min_align_score < top_align_score)
				max_align_score2 = top_align_score;
			
			top_align_score = top_align_score*0.5;
			if (min_align_score < top_align_score)
				min_align_score = top_align_score;
			*/
			
			top_align_score = min_align_score;
			minHashHitsCutoff = maxHit;
			nearTopAs = min_align_score;
