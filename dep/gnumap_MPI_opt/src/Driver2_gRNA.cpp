...
CMDLine this_cmd;
/*****************************************************/
int align_sequence(Genome &genStrand, seq_map &unique, const Read &search, const string &consensus, double min_align_score, double near_max_align_score, double &denominator, double &top_align_score, int strand2ali, int align_mode, int thread_id);
...
void usage(int has_error, char* errmessage) {
	...
	cout << "Usage: gnumap [options] <file_to_parse>\n"
		 ...
		 << "                               (default: mer_size)\n"
		 << "  -i, --pair_id=INT            If the input read header does not follow @xxxxx/1 or @xxxxx/2,\n"
		 << "                               specify the read pair id. [0],1,2\n"
		 /*
		 ...
		 */
		 ...
		 << "  -y, --top_hash=INT           Consider a set of position with the max number of hash hits only (e.g., top y)\n"
		 ...
		 << "  -0, --print_full             Print locatio/s for the entire sequence, not\n"
		 ...
		 << "  -b, --c2t                 Flag to turn on the C to T conversion, used in\n"
		 << "                               bisulfite sequence analysis\n"
		 << "  -B, --g2a                 Convert G to A\n"
		 << "  -d, --a2g                 Flag that allows for A to G conversion\n"
		 << "  -D, --t2c                 Flag that allows for T to C conversion\n"
...
*/
	...
	if (gMATCH_POS_STRAND) {
		...
		poorRead = false;
		cnt_summit = align_sequence(gen, *unique, *search, consensus, 
									min_align_score, near_max_align_score, denominator, top_align_score_pos, POS_STRAND, NW_ALIGN, thread_id);
	}
	...
	if (gMATCH_NEG_STRAND) {
		...
		// append both ends to the original read to get an accurate align transcription
		poorRead=false;
		cnt_summit = align_sequence(gen, *unique, rc, rc_consensus, 
								min_align_score, near_max_align_score, denominator, top_align_score_neg, NEG_STRAND, NW_ALIGN, thread_id);
...
int main(const int argc, const char* argv[]) {
	...
	if(gJUMP_SIZE) {
		...
		gJUMP_SIZE = (int)floor(gMER_SIZE/2);
	}
	...
	*/
	...
	if (gBISULFITE) {
		...
		if(pos_matrix == NULL) {
			//when it aligns to + strand(ref) 
			//in mapping read:'C' to genome:'C'
	...
	if (gGRNA) {
		...
		gALIGN_SCORES[(int)'a'][2] = gALIGN_SCORES[(int)'A'][2] = gMATCH-gMATCH_offset; //we have to turn it off
		...
		gALIGN_SCORES[(int)'c'][2] = gALIGN_SCORES[(int)'C'][2] = gTRANSVERSION; //we have to turn it off
		gALIGN_SCORES[(int)'c'][3] = gALIGN_SCORES[(int)'C'][3] = gMATCH-gMATCH_offset;
		
		gALIGN_SCORES[(int)'g'][0] = gALIGN_SCORES[(int)'G'][0] = gMATCH-gMATCH_offset;
		gALIGN_SCORES[(int)'g'][1] = gALIGN_SCORES[(int)'G'][1] = gTRANSVERSION;
		gALIGN_SCORES[(int)'g'][2] = gALIGN_SCORES[(int)'G'][2] = gMATCH; //we have to turn it off
		gALIGN_SCORES[(int)'g'][3] = gALIGN_SCORES[(int)'G'][3] = gTRANSVERSION;
		
		gALIGN_SCORES[(int)'t'][0] = gALIGN_SCORES[(int)'T'][0] = gTRANSVERSION;
		gALIGN_SCORES[(int)'t'][1] = gALIGN_SCORES[(int)'T'][1] = gMATCH-gMATCH_offset;
		gALIGN_SCORES[(int)'t'][2] = gALIGN_SCORES[(int)'T'][2] = gTRANSVERSION; //we have to turn it off
		gALIGN_SCORES[(int)'t'][3] = gALIGN_SCORES[(int)'T'][3] = gMATCH;
	}
...
//#endif //DEBUG
		...
		for(size_t i=0; i<gen_size; i+=gMEM_BUFFER_SIZE) {
			...
			if(gSNP || gBISULFITE || gATOG || gATOG2) {
#ifdef DISCRETIZE
				if(gVERBOSE > 0)
...
int ParseCmdLine(const int argc, const char* argv[]) {
	...
	for(int i=1; i<argc; i++) {
		if(*argv[i] == '-') { //check for normal
			...
			if(*((argv[i])+1) == '-')  { //check for extended e.g., --xxx
...
int set_arg(const char* param, const char* assign, int &count) {
	...
	switch(*(param+1)) {	//switch on the first character
		...
		case 's':
			...
			break;
		case 'y':
			if(sscanf(assign,"%d",&gTOP_K_HASH) < 1)
				return PARSE_ERROR;
			break;
		case 'n':
		...
		case 'i':
			if(sscanf(assign,"%d",&gPAIR_ID) < 1)
				return PARSE_ERROR;
			ostr << gPAIR_ID;
			gPAIR_ID_STR = ostr.str();
			break;
...
int set_arg_ext(const char* param, const char* assign, int &count) {
	...
	else if(strncmp(param+2,"pair_id=",8) == 0) {
		//which = 'i';
		which = PAR_PAIRID;
		adjust = 9;
	}
	...
	else if(strncmp(param+2,"top_hash=",9) == 0) {
		//which = 'y'
		which = PAR_TOP_HASH;
		adjust = 10;
	}	
...
*/
		...
		case PAR_ILL:
			...
			break;
		case PAR_TOP_HASH:
			if(sscanf(param,"%d",&gTOP_K_HASH) < 1)
				return PARSE_ERROR;
			gTOP_K_HASH--;
			break;
		case PAR_NOQUAL:
...
int align_sequence(Genome &gen, seq_map &unique, const Read &search, const string &consensus, double min_align_score, double near_max_align_score, double &denominator, double &top_align_score, int strand2ali, int align_mode, int thread_id) {

	//min_align_score: the minimum align score or cutoff to be reported and it must be given by user
	//near_max_align_score: this is an alignment score nearly closed to the best alignment score for the read
...
#ifdef DEBUG
	cerr << "i_next=" << i_next << endl;
	cerr << "last_hash=" << last_hash << endl;
#endif
	if (i_next<last_hash) {
	...
	while (true) {
		...
		if (align_mode == NW_ALIGN) {
			...
			if (top_beginning > 0) {
				...
				min_align_score_sw = near_max_align_score * SW_ALIGN_R_CUTOFF;
		...
		for(loc_it = loc_it_begin; loc_it != loc_it_end; ++loc_it) {
			...
			if(align_score >= top_align_score) { 
				...
				if (top_align_score >= near_max_align_score) { //check if this is nearly exact match!
					cnt_top_map+=5*gMAX_MATCHES;
					cnt_summit++;
					
					if ((cnt_summit > gMAX_MATCHES)||(gUNIQUE && (cnt_summit > 1)))
						return cnt_summit;
...
//add print_all_sam flag to enable the feature
				...
				nearTopAs = 2.0 * earlyTerminationScore;
				//-----------------------------------------------
			...
			if(align_score >= nearTopAs) {
				...
				if ((align_mode == NW_ALIGN) && (beginning > gMAX_GAP) && ((beginning + searchL + gMAX_GAP) < gGenSizeBp)) {
					...
					if (!probeE1 && !probeE2) { // m , m
						...
						beginningE = beginning;
						to_matchE = gen.GetString(beginningE, searchL+gMAX_GAP);
						if (to_matchE.size() ==0) {
							to_matchE = to_match;
							probeE2=false;
						}
						...
						to_matchE = gen.GetString(beginningE, searchL+2*gMAX_GAP);
						if(to_matchE.size() == 0) {	//means it's on a chromosome boundary--not valid sequence
							to_matchE = to_match;
							beginningE = beginning;
							probeE1=false;
				...
				if (it == unique.end()) {
					...
					else if(gBISULFITE || gATOG || gBISULFITE2 || gATOG2) {
						temp =
