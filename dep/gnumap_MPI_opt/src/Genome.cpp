...
#ifdef DEBUG
	fprintf(stderr,"[%d] with begin=%lu, my_start=%lu, Assertion checks %lu <= %lu\n",
			iproc,begin,my_start,pos_in_gen,genome_size/GEN_PACK); //debugF
#endif
...
void Genome::AddSeqScore(unsigned long pos, const float amt, unsigned int which) {
	...
	int i;
	fprintf(stderr,"[discretize] At loc %u, read_allot is %d\n",loc,read_allot[loc]);
	center_t *map_c = centers[read_allot[loc]];
	center_t gen_amt[DIM_CENTERS];
	for(i=0; i<DIM_CENTERS; i++)
	...
	unsigned int reads_loc = loc*NUM_SNP_VALS;
	unsigned char newC;
	//fprintf(stderr,"[intdisc] At loc %u, read_allot is %d\n",loc,read_allot[loc]); //debugF
	for(unsigned int i=0; i<NUM_SNP_VALS; i++) {
		if(i == which) {
...
#else
	//fprintf(stderr,"[else] which:%u\tloc:%u\tamt:%f\n",which,loc,amt); //debugF
...
void Genome::PrintFinal(const char* fn, bool append) {
	...
	else if(gBISULFITE || gBISULFITE2 || gATOG || gATOG2) {	// if any of the reads have been instantiated...
	...
	for(unsigned int j=0; j<names.size(); j++) {
		...
		for(; count<names[j+1].second; count+=gGEN_SIZE) {
			...
			if(snp_pos1 != g_gen_CONVERSION[(int)at_pos] || dip) {
				if( p_val < gSNP_PVAL) {
					if(dip) {
						fprintf(vcf_file,"%s\t%lu\tsnp%lu\t%c\t%c%c\t.\t.\tDiploid;pval=%.5f;coverage=%.5f;ratio=%.2f\n",
						...
						fprintf(vcf_file,"%s\t%lu\tsnp%lu\t%c\t%c\t.\t.\tMonoploid;pval=%.5f;coverage=%.5f\n",
								names[j].first.c_str(), (count-names[j].second)+1, snp_count++, 
