 ...
 */
string print(GENOME_t &gen, HashLocation* pos, unsigned long begin, unsigned int *size2) {
	string to_return = "";
	unsigned long *query_start=0;
	//cout << "matches: " << pos.size() << endl;
	unsigned int *chr_num=0;
	for(unsigned long i=0; i<pos->size; i++) {
		*query_start= pos->hash_arr[i] - begin;
		chr_num=0;
		to_return += gen.GetString(*query_start, *size2, *chr_num) + "\t";
	}
	...
	while (true) {
		...
		for(loc_it = loc_it_begin; loc_it != loc_it_end; ++loc_it) {
			...
			if(loc_it->second < minHashHitsCutoff)
				...
				pair<string,unsigned long> seq_pos = gen.GetPosPair(loc_it->first);
				strncpy(ref_name,seq_pos.first.c_str(),MAX_NAME_SZ-1);
				ref_name[MAX_NAME_SZ-1] = '\0';
				fprintf(stderr,"%s\t%u\t%u",ref_name,loc_it->first,loc_it->second);
			}
			beginning = loc_it->first;
			
			//check if this is alreayd computed previously
			chr_num=0;
			to_match = gen.GetString(beginning, searchL, chr_num);
			...
			if(align_score >= nearTopAs) {
				...
				if ((align_mode == NW_ALIGN) && (beginning > gMAX_GAP) && ((beginning + searchL + gMAX_GAP) < gGenSizeBp)) {
					...
					if (!probeE1 && !probeE2) { // m , m
						...
						to_matchE = gen.GetString(beginningE, searchL2, chr_num);
						if (to_matchE.size() ==0) {
							to_matchE = to_match;
							probeE2=false;
						}
