...
Read* SeqReader::GetNextSequence() {
	...
	if(seq_counter >= num_reads) { //once it enters inside, num_reads will be reset and seq_counter is to count reads to load to gArrayRead
		if(this_type == PRB)
...
inline double SeqReader::Q2Prb_ill(double Q) {
	double answer = 1.0 - pow(10.0,(-Q/10.0));
...
bool SeqReader::get_more_fastq() {
	...
	string name, sequence, name_second, fastq, tmp;
	...
	for(unsigned int to_read = 0; to_read < READ_BUFFER; to_read++) {
		...
		// only use first string before encountering ' ' (white space)
		if (gPAIR_ID>0) {
			name = name+"/"+gPAIR_ID_STR;
		}
		...
		while(name[0] != '@' || name_second[0] != '+' || sequence.length() > fastq.length()) {
			fprintf(stderr, "--Found error in [%s]...atteming to resolve...\n",name.c_str());
			//ERROR
		...
		for(int i=0; i<(int)sequence.size(); i++) {
			...
			if(gPHRED_OFFSET==SANGER_PHRED_OFFSET) {
				Q -= SANGER_PHRED_OFFSET;	// Standard method
				max_prb = Q2Prb_std((double)Q);
				
				if(max_prb < 0) {
					max_prb=0.0;
					fprintf(stderr,"max_prb < 0");
					fprintf(stderr,"Found Q of %d and original was %i\n",Q,fastq[i]);
				}
				other_prb = (1 - max_prb) / 3;
			}
			else if (gPHRED_OFFSET==ILLUMINA_PHRED_OFFSET) {
				Q -= ILLUMINA_PHRED_OFFSET;
			...
			else if (gPHRED_OFFSET==NO_FASTQ) {
				max_prb = 1.0;
				other_prb =0.0; 
			}
