...
#
# TODO: to support masking|filtering homopolymer regions instead of filtering
#===============================================================================
...
#        chomp();
 #       next unless(length($_));
        if($count-- == 0) {
            ...
            $fasta = 1;
            $qual = 1;
        } elsif($fasta == 1 && (($aa && /^[ABCDEFGHIKLMNOPQRSTUVWYZXabcdefghiklmmopqrstuvwyzx*-]+/o) || (!$aa && /^[\nACGTURYKMSWBDHVNXacgturykmswbdhvnx-]+/o))) {
            $fasta = 2;
        } elsif($qual == 1 && /^\s*\d+/o) {
            $qual = 2;
        } elsif(!$fastq && /^\@(\S+)\s*/o) {
            $id = $1;
            $fastq = 1;
        } elsif($fastq == 1 && (($aa && /^[ABCDEFGHIKLMNOPQRSTUVWYZXabcdefghiklmmopqrstuvwyzx*-]+/o) || (!$aa && /^[\nACGTURYKMSWBDHVNXacgturykmswbdhvnx-]+/o))) {
            $fastq = 2;
        } elsif($fastq == 2 && /^\+(\S*)\s*/o) {
...
sub readEntryFastq {
    ...
    my ($seq,$seqid,$qual,$header,$line,$tmpid,$S);
    if(defined($line = <$fh>)) {
        $line =~ /^\@(\S+)\s*(.*)$/o;
        $seqid = $1;
        $header = $2 || '';
        ...
        chomp($seq);
				$S=length($seq);
				if ($S==0){ #3 lines: added by cjhong@bu.edu
					$seq="NNNNN";
				}
...
sub readEntryFasta {
    ...
    my ($seq,$seqid,$header,$line,$tmpid,$S);
    $seq = '';
    if($nextid) {
        $seqid = $nextid;
        $header = $nextheader;
