...
sub cmd_blat_pslx {
	...
	my $partThreads = $gTHREADS; #pblat

	my $cmdLine = "cd $unmappedD";
	$cmdLine = "$cmdLine;$gBIN[$bBLAT][$bbBIN] $refIdx -threads=$gTHREADS -minIdentity=$gBIN[$bBLAT][$bbPERIDN] -tileSize=$gBIN[$bBLAT][$bbTILESZ] -stepSize=$gBIN[$bBLAT][$bbSTEPSZ] -repMatch=$gBIN[$bBLAT][$bbREPMAT] $blatOption -out=pslx $readBase.$gBIN[$bBLAT][$bbREAD_FEXT] $readBase.$gOut[$oMEXT][1]\n"; #running pblat
...
sub perSam_core {
	...
	for ($s = 0 ; $s < 2 ; $s++) {    #for each strand
		...
		foreach $chrBase ( @{$Chrs} ) {                     #for each chr
			...
			cmd_merge2peSam($AstrandDfn[0][0],$AstrandDfn[1][1],$peAliDfn,1,$gRead[$rREAD_TYPE],$gRead[$rPER_AVE_DIST],$gRead[$rMAX_FRAG_LEN], $fopInAfter);
			...
			cmd_merge2peSam($AstrandDfn[0][1],$AstrandDfn[1][0],$peAliDfn,2,$gRead[$rREAD_TYPE],$gRead[$rPER_AVE_DIST],$gRead[$rMAX_FRAG_LEN], $fopInAfter);
...
sub parse_arguments {
	...
	#-----------------------------------
	# about fato2bit
	$gBIN[$b2BIT][$b2BIN]      = "$depD/pblat/faToTwoBit"; #pblat
