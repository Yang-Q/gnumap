...
my $outSam = $ARGV[2];
my $queryType = $ARGV[3];
my $numMatchCutoff = $ARGV[4]+0.0;
my $qualOn = $ARGV[5]+0.0;
my $perId = $ARGV[6]+0.0; #-1: single.fq, 0:pair_1.fq, 1:pair_2.fq

my $mapQconst = 255.0/log(100*3);
...
if ($qualOn == 1.0) {
	($ReadSeq,$ReadQual) = read_fastq_in_hash($orgRead,$hReadPsl,$perId);
}
else {
...
# 	}
  ...
  (@qStr,@tStr)=();
  if ($t[1]>0) { #if this mapping is considered to catch nt conversion like C2T or A2G, let us focus on the one that there is mismatch between query and subj
  ...
  if ( $clipLen2 > 0) {
	...
	unless (exists $ReadSeq->[$idx]) {next;}
...
sub read_fastq_in_hash {
	my ($fastqDfn,$hReadPsl,$pId) = @_;
	
	open(BLAT_IN_FQ,"<$fastqDfn") || die "cannot read fasta file that blat used!\n";
	my ($line,$line4,$printMe,$readIdx,$cnt,$readId,$suffix); $cnt=0;
	my @ReadSeq = ();
	$ReadSeq[0]="X";
	...
	while($line = <BLAT_IN_FQ>) {
		...
		if (($printMe==0) && ($line =~ /^@(\S+)/)) {
			...
			if (exists $hReadPsl->{$readId}) {
				$readIdx = $hReadPsl->{$readId};
				$line = <BLAT_IN_FQ>; # seq
				chomp($line);
				$ReadSeq[$readIdx] = $line;
				$line = <BLAT_IN_FQ>; # +
