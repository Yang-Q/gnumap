...
my $map1stR2up = $ARGV[4]; #1 indicates _1 aligned to pos and _2 aligned to neg. Otherwise, use 2!
#my $mapper = $ARGV[5];
...
# my @noInput = (0,0);
unless (-e $infile1) {
	system("touch $infile1");
}
# 
unless (-e $infile2) {
	system("touch $infile2");
}

my $rndTag = int(rand(100000));

$tmp_file = $infile1 . "-temp1-$rndTag";
$x = system("cat $infile1 $infile2 | sort -T $scratchDir2sort -n -k1,3 -o $tmp_file");
if ( $? == -1 ){
  die "command failed: $!\n";
}
...
open($outSamFp, ">$outfile1") or die "\nERROR: in script merge2sam_v3.pl: Cannot open file '$outfile1' for writing\n";
...
sub build_paired_end_core2 {
	...
	my ($cnt,$absDistF,$which,$i,$j,$mapStatus,$diffAveInnerDist);
	my $distTooLong = 0;
	my $peDist;	
	$mapLen[0] = $#{$mPos->[0]}+1;
	$mapLen[1] = $#{$mPos->[1]}+1;
	...
	if ($mapLen[0]>0 && $mapLen[1]>0){
			...
			for ($j=0;$j<$mapLen[1];$j++){ #for each entry of 2nd pair mapping
				...
				if ((($polarity->[0][$i] + $polarity->[1][$j]) == 1) && (($peDist + $readL->[0]) <= $gInsLib[1])) {
					...
					if (($gInsLib[0]<0) && ($readL->[0]<=(-1*$gInsLib[0]))) {
						...
						$diffAveInnerDist = abs($peDist - $gInsLib[0] - $readL->[0]);
					}
		...
		if ($distTooLong != 1) { 
			...
			set_sam_bit_flag($INVALID_DIST,$INVALID_DIST,-1,\@samRep,\@peMapLoc,$mapLib,$mPos,$readL);
		}
	}
	else {
		$which=1;
		...
		@dummy = ((0,0),(0,0));
		set_sam_bit_flag($ONE_MATE_UNMAPPED,$which,0,\@samRep,\@dummy,$mapLib,$mPos,$readL);
	}
...
sub update_sam_print {
	...
	for ($i=0; $i<$j; $i++) {
		...
		#2. mate unmapped
		if (($entr[1]&(1<<$SAM_VALID_PAIR))!=0){
			print $outSamFp join("\t",@entr);
		}
		elsif (($gInclDiscordantF) && (($entr[1]&(1<<$SAM_MATE_UNMAPPED))!=0)) {
			print $outSamFp join("\t",@entr);
		}
	}
}
...
sub set_sam_bit_flag {
	my ($mapStatus,$which,$dimS,$samRep,$peMapLoc,$mapLib,$mPos,$readL) = @_;
	my ($i,$j,$I,$cnt);
	my @I2=(0,0);
	...
	if ($mapStatus==$ONE_MATE_UNMAPPED) {
		...
		$I2[1] = $#{$mPos->[1]}+1;
		$cnt=0;
		for ($j=0;$j<2;$j++){ #for each pair read
			for ($i=0;$i<$I2[$j];$i++) { #for each mapping instance
				$samRep->[$cnt][3] = 0;
				$samRep->[$cnt][4] = 0;
				$samRep->[$cnt][0] = $mPos->[$j][$i]; 
				$samRep->[$cnt][1] |= (1 << $SAM_SEQ_PAIRED);
				$samRep->[$cnt][1] |= (1 << $SAM_MAP2STRAND[$j]);
				$samRep->[$cnt][1] |= (1 << $SAM_IN_PAIR[$j]);
				$samRep->[$cnt][2] = $mapLib->[$j][$peMapLoc->[$i][$j+4]]; 
...
sub one2one_map {
	...
	for ($i=0;$i<$N;$i++) {
		...
		if ($validF) { # 1 indicates valid distance
			...
			if ($vacancy) {
				#check if flip PER get this pos prevly
				$key="$peMapLoc->[$i][3]:$peMapLoc->[$i][2]";
				$prevFlip=$hPairLoc{$key};
				$vacancy=0;
				if (!defined($prevFlip)){
					$hPairLoc{$key}=1;
					$vacancy=1;
	...
	if ($cnt==0) {#woops, too far from each other
		...
		set_sam_bit_flag($mapStatus,-1,$cnt,\@samRep,$peMapLoc,$mapLib,-1);
	}
