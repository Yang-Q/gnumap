...
push @usage, " --num_amb			speicify the number of mapping locations to be reported [50]\n";
...
my ( $help, $genome, $transc, $pair_1, $pair_2, $single, $qcontrol, $adap, $adap2, $lib_type, $read_type, $phred_offset, $per_dist, $discordant, $nt_conv, $pileup1, $map_quality, $num_amb, $skip_blat, $mpi, $num_threads, $outdir, $debug_mode);
...
if (not defined $num_amb){$num_amb=50;}
if (not defined $debug_mode){$debug_mode=0;}

########################################
# check input condition and requirements
########################################
my ( $read1, $read2, $installD ) = parse_arguments($genome,$transc,$pair_1,$pair_2,$single,$qcontrol,$adap,$adap2,$lib_type,$read_type,$phred_offset, $per_dist, $discordant, $skip_blat, $nt_conv, $map_quality, $num_amb, $mpi, $num_threads,$outdir,$debug_mode);
...
my $gnuAln = postprocessing( \@Chrs, $gBIN[$bGNU][$bgOPRFX], $gBIN[$bGNU][$bgGNU2DSC], $gnuOutPrefix, $gOut[$oSTRAND][0], $gOut[$oSTRAND][1], $gOut[$oPI][$gRead[$rPER]+1] );
...
if ($skip_blat==0) {
	($isEmpty,$unmappedD) = get_unampped_reads( $gnuOutPrefix, $gnuAln, $gBIN[$bBLAT][$bbREAD_FEXT] );
...
sub run_pblat_gen_tran3 {
	...
	$blatTGenAln = postprocessing( \@Chrs, $gBIN[$bBLAT][$bbOPRFX], $gBIN[$bBLAT][$bb2DSC], $blatGenSamD, $subD2opt, 'X', $gOut[$oPI][$gRead[$rPER]+1]);
...
sub pBLAT_wrapper {
	...
	for ( $p = 0 ; $p < $gRead[$rPER] ; $p++ ) {# for each paired read (for single end, only _1.txt will be processed)
		...
		unless (-e $blatSamDfn) {
			...
			$fqDfn = "$faD/$pi.$gRead[$rFEXT]";
			$cmd = "$gBIN[$bPSL2SAM][$bpsBIN] $fqDfn $pslDfn $blatSamDfn $psl2samOpt $numMatchCutoff 1 $p\n";
			#print "$cmd\n"; #debug
			system($cmd);
		}
		...
		if ( $onlyBest == 1 ) {
			...
			cmd_resolve_ambig_sam2( $gBIN[$bBLAT][$bbOPRFX], $gOut[$oPI][2], $blatSamDfn, $alignBestDfn, $gBIN[$bBEST][$b1NUM_AMB], $handleInAfter, 0, $updXP_field); #!debug
			if (-e $alignBestDfn){
...
sub postprocessing {
	my ( $Chrs, $mapper, $discordant, $samDprefix, $subD2opt1, $subD2opt2, $perTag) = @_;
	...
	if ( $gRead[$rPER] == 2 ) {            #merge in per mode
		...
		perSam_core( $Chrs, $samDprefix, $subD2opt1, $subD2opt2, $discordant );
	}
	...
	my $updXP_field = 1;
	cmd_resolve_ambig_sam2( $mapper, $perTag, $alignMergedSamDfn, $alignBestDfn, $gBIN[$bBEST][$b1NUM_AMB], $handleInAfter, 1, $updXP_field); #!debug
...
sub get_unampped_reads {
	...
	my ( $samDprefix, $alnDfn, $outFmt ) = @_;    #get a hit sam file
	...
	my @UnmappedPe = cmd_extractUnmappedPeRead(\@OrgRead, $alnDfn, $outFmt, $gnuUnmapD);
...
sub cmd_extractUnmappedPeRead {
	my ($OrgRead, $gnuSamConcoDf, $outFmt, $gnuUnmapD ) = @_;
	...
	my $cmdLine = "$gBIN[$bUNMAP2FA][$buBIN] $readD $inFmt $outFmt $OrgRead->[0] $OrgRead->[1] $gnuSamConcoDf $gnuUnmapD\n";
	#print "$cmdLine\n"; #debug
	system($cmdLine);

	$outFmt = "fq";
	$cmdLine = "$gBIN[$bUNMAP2FA][$buBIN] $readD $inFmt $outFmt $OrgRead->[0] $OrgRead->[1] $gnuSamConcoDf $gnuUnmapD\n";
	#print "$cmdLine\n"; #debug
	system($cmdLine);
...
sub cmd_gnumap {
	...
	$cmdLine = "$cmdLine\n$gBIN[$bGNU][$bgBIN] -g $refDfn --read=$refIdx -o $aliDfx -m $gBIN[$bGNU][$bgGNU_KMER] -s $gBIN[$bGNU][$bgGNU_SLIDE] -c $gTHREADS -h $gBIN[$bGNU][$bgGNU_MAX_HASH] -t $gBIN[$bBEST][$b1NUM_AMB] -a $gBIN[$bGNU][$bgGNU_ACC] $gnuNtConvOpt $read_pair_id_opt -y $gBIN[$bGNU][$bgGNU_TOPK_HASH] $read\n";
...
#===========================================================================
sub perSam_core {
	my ( $Chrs, $samDprefix,  $subD2opt1, $subD2opt2, $allowDiscordant ) = @_;
	...
	for ($s = 0 ; $s < 2 ; $s++) {    #for each strand
		...
		foreach $chrBase ( @{$Chrs} ) {                     #for each chr
			...
			cmd_merge2peSam($AstrandDfn[0][0],$AstrandDfn[1][1],$peAliDfn,1,$gRead[$rREAD_TYPE],$gRead[$rPER_AVE_DIST],$gRead[$rMAX_FRAG_LEN], $allowDiscordant, $fopInAfter);
			...
			cmd_merge2peSam($AstrandDfn[0][1],$AstrandDfn[1][0],$peAliDfn,2,$gRead[$rREAD_TYPE],$gRead[$rPER_AVE_DIST],$gRead[$rMAX_FRAG_LEN], $allowDiscordant, $fopInAfter);
...
sub cmd_merge2peSam {
	my ( $samA, $samB, $peDf, $pairAlignMode, $refType, $avePerDist,
	  ...
	  = @_;
	my $progCmd = "";
	my $allowDiscordant = 1;
	
	
	if (($mapper eq "gnumap") || ($gRead[$rCONV] eq "a2i")) {
		$allowDiscordant = 0;
	}
...
sub cmd_resolve_ambig_sam2 {
	my ( $mapper, $per, $ambSam, $bestSam, $numAmb, $handleInAfter, $updFlag, $updXP_field ) = @_;
...
# 	if ( $mapper eq "blat" ) { $mapperId = 2; }
	...
	my $cmdLine ="$gBIN[$bBEST][$b1BIN] $mapper $per $gnuNtConvF $ambSam $bestSam $gOut[$oTEMPD] $numAmb $updFlagMmap $updXP_field\n";
...
sub parse_arguments {
	my (
		...
		$lib_type,   $read_type,      $phred_offset, $per_ave_dist, $discordant, $skip_blat,
	...
	$gPID_IN_HEAD = check_fastq_pid_head_format($read0);
