...
if ($queryType eq "rna") { # for RNA, lowe penalty for deletion
	@q = (4,4);
	@r = (2,0);
}
...
while ($line = <PSL>) {
  ...
  for ($i=1;$i<$t[17];$i++) { #for each block
  ...
  if ( $clipLen2 > 0) {
	...
	$idx = $hReadPsl->{$s[0]};
	$s[9] = $ReadSeq->[$idx];
	$cutoffScore = (length($ReadSeq->[$idx]) - $intronLen) * $a * $aPerCutoff;

	if ($ntConvF && ($t[1]>0)) {
		$mmScore = $As[0];
		if ($As[0]<$As[1]) {$mmScore = $As[1];}
  ...
  $s[11] = "AS:i:$score";
	...
	if ($t[8] eq '-') {
		$s[9] = reverse_complement($ReadSeq->[$idx]);
	}
...
sub comp2Strs {
	my ($qStr,$tStr,$matchSc,$mismatchSc,$As) = @_;
