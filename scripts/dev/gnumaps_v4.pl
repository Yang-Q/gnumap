...
push @usage, " --num_amb			speicify the number of mapping locations to be reported [100]\n";
...
if (not defined $num_amb){$num_amb=100;}
...
if ($skip_blat==0) {
	...
	($isEmpty,$faReadD) = conv_fastq2fasta(\@Reads);
	##########################################
	# BLAT aligns unmapped reads to both genome
	#  and transcriptome
	##########################################
	if ($isEmpty==0) {
		$blatSamMappedFn =run_pblat_gen_tran3( $blatGenRefIdx, $blatTrnRefIdx, $faReadD,$gRef[$gGNREF] );
	}
}
...
sub pBLAT_wrapper {
	...
	for ( $p = 0 ; $p < $gRead[$rPER] ; $p++ ) {# for each paired read (for single end, only _1.txt will be processed)
		...
		unless (-e $blatSamDfn) {
			...
			$cmd = "$gBIN[$bPSL2SAM][$bpsBIN] $faDfn $pslDfn $blatSamDfn $psl2samOpt $gRead[$rCONV] $numMatchCutoff 0 $p $gRead[$rLIB_TYPE]\n";
			print "$cmd\n"; #debug
			system($cmd);
		}
...
sub cmd_gnumap {
	...
	$cmdLine = "$cmdLine\n$gBIN[$bGNU][$bgBIN] -g $refDfn --read=$refIdx -o $aliDfx -m $gBIN[$bGNU][$bgGNU_KMER] -s $gBIN[$bGNU][$bgGNU_SLIDE] -c $gTHREADS -h $gBIN[$bGNU][$bgGNU_MAX_HASH] -t 25 -u -a $gBIN[$bGNU][$bgGNU_ACC] $gnuNtConvOpt -y $gBIN[$bGNU][$bgGNU_TOPK_HASH] $read\n";
...
#===================
sub conv_fastq2fasta {
	my ($Reads) = @_;
	my @Read2s=['x','x'];
	my ($faReadD,$cmd,$p,$fileBase);
	
	for ( $p=0; $p<$gRead[$rPER]; $p++) {
		($faReadD,$fileBase,$gDummy)=parse_filename($Reads[$p]);
		$Read2s[$p]="${faReadD}/${fileBase}.$gBIN[$bBLAT][$bbREAD_FEXT]";
		$cmd="sed -n \'1~4s/^\@/>/p;2~4p\' $Reads->[$p] > $Read2s[$p]";
		print STDOUT "$cmd\n";
		system($cmd);
	}
	
	($faReadD,$gDummy,$gDummy)=parse_filename($Reads[0]);
